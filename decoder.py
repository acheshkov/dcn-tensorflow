...
def decoderBatch(U, lstm_dec, dropout_rate, FLAGS):
    ...
    with tf.name_scope('DYNAMIC_POINTING_DECODER'):
        ...
        start_pos = tf.constant(0, shape=tf.TensorShape(batch_size), dtype=tf.int32)
        end_pos = tf.constant(0, shape=tf.TensorShape(batch_size), dtype=tf.int32)
        ...
        for i_ in range(max_decoder_iterations):
            scores_start, scores_end, new_start_pos, new_end_pos, lstm_dec_state = decoderIteration(U, 
                                                                                                    lstm_dec_state, 
                                                                                                    start_pos, 
                                                                                                    end_pos,
                                                                                                    lstm_dec,
...
'''
def decoderIteration(U, lstm_state, start_pos, end_pos, lstm_dec, dropout_rate, max_sequence_length, lstm_size, FLAGS, iter_number):
    # returns (batch, 2L)
    def _getPos(U, start_positions, rows_size, vec_size):
        ...
        # (batch, 2L)
        return tf.reduce_sum(positions, 1);
